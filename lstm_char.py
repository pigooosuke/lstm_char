...
def get_bathes(arr, n_seqs, n_steps):
    ...
    '''
    characters_per_batches = n_seqs * n_steps
    n_batches = len(arr) // characters_per_batches

    arr = arr[:characters_per_batches * n_batches]
...
def pick_top(preds, vocab_size, top_n=5):
    p = np.squeeze(preds)
    p[np.argsorts(p)[:-top_n]] = 0
    p = p / np.sum(p)
    c = np.random.choice(vocab_size, 1, p=p)[0]
    return c

def sample(checkpoint, vocab, n_samples, lstm_size, vocab_size, prime="The "):
    sample = [c for c in prime]
...
def test(vocab):
    ...
    samp = sample(checkpoint, vocab, 1000, lstm_size, len(vocab), prime="Far")
    print(samp)
